<template>
  <div class="flex-1 min-h-screen items-center justify-center bg-neutral-100">
    <Form
      @submit="handleSubmit($event, currentEditingFolder)"
...
async function handleSubmit (values: any, targetFolder : string) {
  console.log('handleSubmit: ' + targetFolder)
  console.log('handleSubmit2: ' + currentEditingFolder)
  toggleIsInput(targetFolder)
  console.log(values)
  switch (targetFolder) {
    case 'profileHead':
...
function toggleIsInput (FolderLabel: string) {
  ...
  currentEditingFolder = FolderLabel
  switch (FolderLabel) {
    case 'profileHead':
