...
public class EgressoPubControllerTest {
    ...
    void testConsultarEgressos() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get("/publico/egresso"))
                ...
                .andExpect(MockMvcResultMatchers.jsonPath("$[0].matricula").value(egressoModel.getMatricula()))
                .andExpect(MockMvcResultMatchers.jsonPath("$[0].nascimento")
                        .value(egressoModel.getNascimento().format(DateTimeFormatter.ofPattern("yyyy-MM-dd"))))
                .andExpect(MockMvcResultMatchers.jsonPath("$[0].genero.nome").value(egressoModel.getGenero().getNome()))
    ...
    void testFindById() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get("/publico/egresso/1"))
                .andExpect(MockMvcResultMatchers.jsonPath("$.usuario.nome")
                        .value(egressoModel.getUsuario().getNome()))
                .andExpect(MockMvcResultMatchers.jsonPath("$.matricula").value(egressoModel.getMatricula()))
                .andExpect(MockMvcResultMatchers.jsonPath("$.nascimento")
                        .value(egressoModel.getNascimento().format(DateTimeFormatter.ofPattern("yyyy-MM-dd"))))
                .andExpect(MockMvcResultMatchers.jsonPath("$.genero.nome").value(egressoModel.getGenero().getNome()))
