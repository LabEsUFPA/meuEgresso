...
public class PesquisaCientificaController {
	...
	/**
	 * Endpoint responsável por retornar a lista de pesquisaCientifica cadastrados no banco de dados.
	 * 
	 ...
	 *
	 * @param pesquisaCientificaDTO Estrutura de dados contendo as informações necessárias para persistir o pesquisaCientifica.
	 ...
	 * 
	 * @param pesquisaCientificaDTO Estrutura de dados contendo as informações necessárias para
	 *                   atualizar o pesquisaCientifica.
	 * @return {@link PesquisaCientificaDTO} Dados gravados no banco com a Id atualizada.
	 * @author Alfredo Gabriel
	 * @since 21/04/2023
	 */
	@PutMapping
	@ResponseStatus(code = HttpStatus.CREATED)
	public PesquisaCientificaDTO atualizarPesquisaCientifica(@RequestBody @Valid PesquisaCientificaDTO pesquisaCientificaDTO) {
		PesquisaCientificaModel pesquisaCientificaModel = mapper.map(pesquisaCientificaDTO, PesquisaCientificaModel.class);
		pesquisaCientificaModel = pesquisaCientificaService.save(pesquisaCientificaModel);
		return mapper.map(pesquisaCientificaModel, PesquisaCientificaDTO.class);
	}
